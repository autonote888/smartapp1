...
if not st.session_state.logged_in:
    ...
    with st.container():
        ...
        if st.button("MASUK SEKARANG"):
            # Cek data ke tabel pegawai
            try:
                res = supabase.table("pegawai").select("*").eq("email", email_input).eq("password", pass_input).execute()
                
                if len(res.data) > 0:
                    st.session_state.user_info = res.data[0] # Simpan data user ke session
    ...
    # --- 5. DASHBOARD USER (DENGAN SAFETY CHECK) ---
    if "user_info" in st.session_state:
        ...
        try:
            ...
            if len(res_pay.data) > 0:
                ...
                st.warning("Data riwayat gaji belum tersedia.")
        except:
            st.error("Gagal mengambil data payroll.")
        ...
        # Jika session hilang tiba-tiba, paksa login ulang
        st.session_state.logged_in = False
        st.rerun()
